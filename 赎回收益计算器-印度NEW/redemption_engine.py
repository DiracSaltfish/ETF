...
# A one-share XOP execution tail is operationally immaterial for a normal
# redemption hedge, but must also be small relative to the basket.  Keeping
# both limits prevents a one-share miss on a tiny hedge from being accepted.
IB_SETTLEMENT_TAIL_MAX_SHARES = 1
IB_SETTLEMENT_TAIL_MAX_RATIO = Decimal("0.002")
...
def is_immaterial_ib_tail(shortfall: int, hedge_target: int) -> bool:
    """Return whether an IB quantity shortfall may be accepted as a tail."""
    if shortfall <= 0 or hedge_target <= 0:
        return False
    return (
        shortfall <= IB_SETTLEMENT_TAIL_MAX_SHARES
        and Decimal(shortfall) / Decimal(hedge_target) <= IB_SETTLEMENT_TAIL_MAX_RATIO
    )
...
class VenueClose:
    ...
    @property
    def ib_matched_qty(self) -> int:
        return min(self.ib_open_qty, self.ib_close_qty)
...
class BasketResult:
    ...
    @property
    def ib_matched_qty(self) -> int:
        """Quantity used for realized IB P&L: equal quantity on both legs."""
        return min(self.ib_open_qty, self.ib_close_qty)

    @property
    def ib_open_tail_accepted(self) -> bool:
        return is_immaterial_ib_tail(self.ib_open_shortfall, self.hedge_target)

    @property
    def ib_close_tail_accepted(self) -> bool:
        return is_immaterial_ib_tail(self.ib_close_shortfall, self.hedge_target)

    @property
    def has_material_ib_shortfall(self) -> bool:
        return (
            bool(self.ib_open_shortfall) and not self.ib_open_tail_accepted
        ) or (
            bool(self.ib_close_shortfall) and not self.ib_close_tail_accepted
        )
...
@dataclass(frozen=True)
class FileSignature:
    """Identity used to invalidate a parsed input after replace or in-place writes."""
...
class CalculationInputs:
    """Immutable parsed inputs; business allocation always starts from this snapshot."""

    qmt_records: tuple[QmtRecord, ...]
    raw_ib_trades: tuple[IbTrade, ...]
    borrow_fees: tuple[BorrowFee, ...]
    ib_stock_trades: tuple[IbStockTrade, ...]
    configured_qmt_sources: frozenset[str]
...
def file_signature(path: Path | str) -> FileSignature:
    """Return a replace-aware signature without reading the file contents."""
...
class CalculationInputCache:
    """Cache immutable file parsing while always rerunning the business pipeline."""

    def __init__(self) -> None:
        self._qmt_cache: dict[str, tuple[FileSignature, tuple[QmtRecord, ...]]] = {}
        self._ib_cache: tuple[
            FileSignature,
            tuple[IbTrade, ...],
            tuple[BorrowFee, ...],
            tuple[IbStockTrade, ...],
        ] | None = None
        self._hint_cache: tuple[
            tuple[str, tuple[date, ...], tuple[FileSignature, ...]],
            dict[tuple[str, date, str, str, int], datetime],
        ] | None = None

    def clear(self) -> None:
        self._qmt_cache.clear()
        self._ib_cache = None
        self._hint_cache = None

    def _load_qmt_source(
        self,
        source: str,
        path: Path | str | None,
    ) -> tuple[QmtRecord, ...]:
        if path is None or not str(path).strip():
            self._qmt_cache.pop(source, None)
            return ()
        try:
            signature = file_signature(path)
        except FileNotFoundError:
            # Preserve load_qmt_file's source-specific error message.
            return tuple(load_qmt_file(path, source))
        cached = self._qmt_cache.get(source)
        if cached is not None and cached[0] == signature:
            return cached[1]
        records = tuple(load_qmt_file(path, source))
        self._qmt_cache[source] = (signature, records)
        return records
    ...
    def _hint_cache_key(
        ...
        days = tuple(sorted(trade_days))
        if root is None or not str(root).strip():
            return "", days, ()
        root_path = Path(root).expanduser().resolve()
        if not root_path.exists():
            return str(root_path), days, ()
        signatures: list[FileSignature] = []
        for path in _qmt_time_hint_files(root_path, trade_days):
            try:
                signatures.append(file_signature(path))
            except FileNotFoundError:
                # Atomic replacements can briefly remove a candidate. The next
                # calculation observes its new signature and reparses it.
                continue
        return str(root_path), days, tuple(signatures)

    def _load_time_hints(
        self,
        root: Path | str | None,
        trade_days: set[date],
    ) -> dict[tuple[str, date, str, str, int], datetime]:
        key = self._hint_cache_key(root, trade_days)
        if self._hint_cache is not None and self._hint_cache[0] == key:
            return self._hint_cache[1]
        hints = _load_qmt_time_hints(root, trade_days)
        self._hint_cache = (key, hints)
        return hints

    def _load_ib(
        self,
        path: Path | str,
    ) -> tuple[tuple[IbTrade, ...], tuple[BorrowFee, ...], tuple[IbStockTrade, ...]]:
        try:
            signature = file_signature(path)
        except FileNotFoundError:
            # Preserve the public loaders' existing error text.
            trades, fees = load_ib_statement(path)
            return tuple(trades), tuple(fees), tuple(load_ib_stock_trades(path))
        if self._ib_cache is not None and self._ib_cache[0] == signature:
            return self._ib_cache[1], self._ib_cache[2], self._ib_cache[3]
        trades, fees = load_ib_statement(path)
        stock_trades = load_ib_stock_trades(path)
        cached = (signature, tuple(trades), tuple(fees), tuple(stock_trades))
        self._ib_cache = cached
    ...
    def load(
        ...
        configured_sources = frozenset(
            source
            for source, path in qmt_paths.items()
            if path is not None and str(path).strip()
        )
        raw_records: list[QmtRecord] = []
        for source in sorted(qmt_paths):
            raw_records.extend(self._load_qmt_source(source, qmt_paths[source]))
        trade_days = {item.trade_day for item in raw_records}
        hints = self._load_time_hints(qmt_time_root, trade_days)
        records = (
            [_enrich_qmt_record_time(item, hints) for item in raw_records]
            if hints
        ...
        raw_ib_trades, borrow_fees, ib_stock_trades = self._load_ib(ib_path)
        return CalculationInputs(
            qmt_records=tuple(records),
            raw_ib_trades=raw_ib_trades,
            borrow_fees=borrow_fees,
            ib_stock_trades=ib_stock_trades,
            configured_qmt_sources=configured_sources,
        )
...
def load_calculation_inputs(
    ...
    """Compatibility loader for callers that do not retain a cache instance."""

    return CalculationInputCache().load(qmt_paths, ib_path, qmt_time_root)
...
def _ib_leg_totals_for_qty(
    slices: Iterable[IbSlice],
    target_qty: int,
) -> tuple[Decimal, Decimal]:
    """Return FIFO-prorated gross and commission for at most target_qty."""
    remaining = max(0, target_qty)
    gross = Decimal("0")
    commission = Decimal("0")
    for item in slices:
...
def _closed_short_pnl(
    opening: Iterable[IbSlice],
    closing: Iterable[IbSlice],
) -> tuple[int, Decimal]:
    """Calculate realized short P&L using equal quantities on both legs.

    A quantity shortfall must never leave a full opening sale in P&L without
    its matching buy.  Any unmatched tail remains auditable as a shortfall but
    contributes neither proceeds nor an invented closing cost.
    """
    opening_slices = tuple(opening)
    closing_slices = tuple(closing)
    matched_qty = min(
        sum(item.qty for item in opening_slices),
        sum(item.qty for item in closing_slices),
    )
    sell_gross, sell_commission = _ib_leg_totals_for_qty(opening_slices, matched_qty)
    buy_gross, buy_commission = _ib_leg_totals_for_qty(closing_slices, matched_qty)
    pnl = sell_gross - sell_commission - buy_gross - buy_commission
    return matched_qty, pnl


def _apply_ib_pnl_to_venue_close(close: VenueClose, fx_rate: Decimal) -> None:
    _matched_qty, close.ib_trade_pnl_usd = _closed_short_pnl(close.ib_open, close.ib_close)
...
def allocate_ib(
    ...
    for moment, kind, payload in events:
        ...
        _matched_qty, basket.ib_trade_pnl_usd = _closed_short_pnl(
            basket.ib_open,
            basket.ib_close,
        )
...
def finalize_baskets(
    ...
    for basket in baskets:
        ...
        if basket.ib_open_shortfall:
            if basket.ib_open_tail_accepted:
                warnings.append(
                    f"IB开仓尾差 {basket.ib_open_shortfall:,} 股已放行；"
                    f"盈亏仅按实际闭合 {basket.ib_matched_qty:,} 股计算"
                )
            else:
                warnings.append(f"IB开仓缺口 {basket.ib_open_shortfall:,} 股")
        if basket.ib_close_shortfall:
            if basket.ib_close_tail_accepted:
                warnings.append(
                    f"IB平仓尾差 {basket.ib_close_shortfall:,} 股已放行；"
                    f"盈亏仅按实际闭合 {basket.ib_matched_qty:,} 股计算"
                )
            else:
        ...
        elif basket.has_material_ib_shortfall:
...
def calculate(
    ...
    input_cache: CalculationInputCache | None = None,
) -> CalculationResult:
    inputs = (
        input_cache.load(qmt_paths, ib_path, qmt_time_root)
        if input_cache is not None
        else load_calculation_inputs(qmt_paths, ib_path, qmt_time_root)
    )
    return calculate_from_inputs(
        inputs,
        fx_rate,
        overrides,
        market_holidays,
...
def calculate_from_inputs(
    ...
    # Lists are recreated for every run. Downstream allocation may replace or
    # extend these containers, while the cached immutable records stay pristine.
    records = list(inputs.qmt_records)
    raw_ib_trades = list(inputs.raw_ib_trades)
    borrow_fees = list(inputs.borrow_fees)
    ...
    ib_stock_trades = list(inputs.ib_stock_trades)
    ...
    if "QMT2" not in inputs.configured_qmt_sources:
        ...
        rows.append(
            {
                ...
                "IB匹配股数": basket.ib_matched_qty,
                "IB开仓股数": basket.ib_open_qty,
                "IB平仓股数": basket.ib_close_qty,
                "IB开仓尾差": basket.ib_open_shortfall,
                "IB平仓尾差": basket.ib_close_shortfall,
                "IB尾差放行": "是" if (
                    basket.ib_open_tail_accepted or basket.ib_close_tail_accepted
                ) else "否",
