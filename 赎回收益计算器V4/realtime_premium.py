...
当 XOP TWS Bid/Ask、159518 新浪五档或 CFETS 汇率任一有效值更新，并且三项数据均完整时，程序会在 500ms 窗口内合并更新并覆盖写入最新快照；
相同数值的重复轮询不会重复写盘，完整估值状态下每 15 秒写入一次心跳快照以更新 `generated_at`。
...
_SCHEMA_DOCUMENT_DIGEST = hashlib.sha256(REALTIME_SCHEMA_DOCUMENT.encode("utf-8")).hexdigest()
_SCHEMA_DOCUMENT_CACHE_LOCK = threading.RLock()
_SCHEMA_DOCUMENT_CACHE: dict[str, tuple[str, tuple[int, int, int, int]]] = {}
...
def _document_file_signature(path: Path) -> tuple[int, int, int, int] | None:
    try:
...
def _clear_schema_document_cache() -> None:
    """Clear the process-local schema cache (used by tests and explicit reloads)."""
    with _SCHEMA_DOCUMENT_CACHE_LOCK:
        _SCHEMA_DOCUMENT_CACHE.clear()
...
def write_schema_document(shared_root: Path | str, *, generated_at: datetime | None = None) -> Path:
    ...
    with _SCHEMA_DOCUMENT_CACHE_LOCK:
        signature = _document_file_signature(document_path)
        cached = _SCHEMA_DOCUMENT_CACHE.get(cache_key)
        if signature is not None and cached == (_SCHEMA_DOCUMENT_DIGEST, signature):
        ...
        if signature is not None:
            ...
            if existing_document == REALTIME_SCHEMA_DOCUMENT:
                _SCHEMA_DOCUMENT_CACHE[cache_key] = (_SCHEMA_DOCUMENT_DIGEST, signature)
                return document_path

        _atomic_write_text(document_path, REALTIME_SCHEMA_DOCUMENT)
        written_signature = _document_file_signature(document_path)
        if written_signature is not None:
            _SCHEMA_DOCUMENT_CACHE[cache_key] = (_SCHEMA_DOCUMENT_DIGEST, written_signature)
    return document_path
...
class TwsXopMarketData(QObject):
    ...
    accountStateChanged = pyqtSignal(object)
    ...
    def __init__(
        ...
        *,
        ...
        self._xop_positions: dict[str, int] = {}
        self._xop_external_pending_sells: dict[str, int] = {}
        self._session_sell_commitments: dict[str, tuple[str, int]] = {}
    ...
    def _account_state_payload(self) -> dict[str, object]:
        with self._lock:
            accounts = tuple(self._managed_accounts)
            positions = dict(self._xop_positions)
            external_pending = dict(self._xop_external_pending_sells)
            session_commitments: dict[str, int] = {}
            for account, quantity in self._session_sell_commitments.values():
                session_commitments[account] = session_commitments.get(account, 0) + quantity
        states: dict[str, dict[str, int]] = {}
        for account in accounts:
            position = positions.get(account, 0)
            pending = external_pending.get(account, 0)
            committed = session_commitments.get(account, 0)
            states[account] = {
                "position": position,
                "pending_sell": pending,
                "session_sell": committed,
                "projected_position": position - pending - committed,
            }
        return {"accounts": accounts, "states": states}

    def account_state(self) -> dict[str, object]:
        return self._account_state_payload()

    def _emit_account_state(self) -> None:
        try:
            self.accountStateChanged.emit(self._account_state_payload())
        except RuntimeError:
            pass

    def _reserve_creation_sell(self, spec: xop_close_orders.XopCloseOrderSpec) -> str | None:
        if spec.intent is not xop_close_orders.XopCloseIntent.CREATION_SELL:
            return None
        if not spec.account:
            return "申购卖出必须先选择IB账户"
        with self._lock:
            if spec.account not in self._managed_accounts:
                return f"IB账户 {spec.account} 不在当前TWS可用账户列表中"
            self._session_sell_commitments[spec.order_ref] = (spec.account, spec.quantity)
        return None

    def _release_creation_sell(self, spec: xop_close_orders.XopCloseOrderSpec) -> None:
        if spec.intent is not xop_close_orders.XopCloseIntent.CREATION_SELL:
            return
        with self._lock:
            self._session_sell_commitments.pop(spec.order_ref, None)
        self._emit_account_state()

    def _sync_creation_sell_commitment(self, trade) -> None:
        """Keep the projected position aligned with partial fills and terminal orders."""
        order_ref = str(getattr(trade.order, "orderRef", "") or "")
        if not order_ref.startswith(xop_close_orders.XopCloseIntent.CREATION_SELL.order_ref_prefix):
            return
        status = str(getattr(trade.orderStatus, "status", "") or "")
        remaining = max(0, int(float(getattr(trade.orderStatus, "remaining", 0) or 0)))
        terminal = status in {"Filled", "Cancelled", "ApiCancelled", "Inactive"}
        with self._lock:
            tracked = self._session_sell_commitments.get(order_ref)
            if tracked is None:
                return
            account, previous_quantity = tracked
            if terminal:
                self._session_sell_commitments.pop(order_ref, None)
                changed = True
            elif remaining > 0 and remaining != previous_quantity:
                self._session_sell_commitments[order_ref] = (account, remaining)
                changed = True
            else:
                changed = False
        if changed:
    ...
    def submit_confirmed_order(self, spec: xop_close_orders.XopCloseOrderSpec) -> bool:
        ...
        with self._lock:
            duplicate = (
                spec.order_ref in self._submitted_order_refs
                or spec.order_ref in self._queued_order_refs
            )
        if duplicate:
            self._emit_order_event(
                {"event": "rejected", "order_ref": spec.order_ref, "message": "orderRef已在本次会话排队或发送，禁止重复"}
            )
            return False
        reservation_error = self._reserve_creation_sell(spec)
        if reservation_error:
            self._emit_order_event(
                {"event": "rejected", "order_ref": spec.order_ref, "message": reservation_error}
            )
            return False
        with self._lock:
            if (
                ...
                self._queued_order_refs.add(spec.order_ref)
                duplicate = False
        if duplicate:
            self._release_creation_sell(spec)
            self._emit_order_event(
                {"event": "rejected", "order_ref": spec.order_ref, "message": "orderRef已在本次会话排队或发送，禁止重复"}
            )
            return False
        ...
        def run() -> None:
            ...
            try:
                ...
                def emit_trade_event(event_name: str, trade, message: str = "") -> None:
                    order_ref = str(getattr(trade.order, "orderRef", "") or "")
                    if not xop_close_orders.is_xop_close_order_ref(order_ref):
                        return
                    self._sync_creation_sell_commitment(trade)
                    self._emit_order_event(
                        {
                            "event": event_name,
                            "order_ref": order_ref,
                ...
                try:
                    open_trades = self._request_all_open_trades(ib)
                    for open_trade in open_trades:
                        existing_ref = str(getattr(open_trade.order, "orderRef", "") or "")
                        if xop_close_orders.is_xop_close_order_ref(existing_ref):
                            with self._lock:
                                self._submitted_order_refs.add(existing_ref)
                    self._refresh_account_state(ib, contract, open_trades=open_trades)
                except Exception as exc:
                    self._emit_order_event(
                ...
                previous = None
                account_refresh_tick = 0
                while ib.isConnected() and not self._stop_event.is_set():
                    ib.sleep(0.1)
                    self._process_order_queue(ib, contract)
                    account_refresh_tick += 1
                    if account_refresh_tick >= 10:
                        self._refresh_account_state(ib, contract)
                        account_refresh_tick = 0
    ...
    def _clear_order_queue(self, message: str) -> None:
        while True:
            try:
                ...
                break
            with self._lock:
                self._queued_order_refs.discard(spec.order_ref)
            self._release_creation_sell(spec)
            self._emit_order_event(
    ...
    def _process_order_queue(self, ib, contract) -> None:
        ...
        try:
            ...
            with self._lock:
                duplicate = spec.order_ref in self._submitted_order_refs
            if duplicate:
                raise ValueError("orderRef已存在，禁止重复发送")
            if spec.intent is xop_close_orders.XopCloseIntent.CREATION_SELL:
                open_trades = self._request_all_open_trades(ib)
                self._refresh_account_state(ib, contract, open_trades=open_trades)
                open_refs = {
                    str(getattr(trade.order, "orderRef", "") or "")
                    for trade in open_trades
                }
                if spec.order_ref in open_refs:
                    raise ValueError("TWS全量未完成订单中已存在相同orderRef")
                with self._lock:
                    account_available = spec.account in self._managed_accounts
                if not account_available:
                    raise ValueError(f"IB账户 {spec.account} 已不在当前TWS可用账户列表中")
    ...
    @staticmethod
    def _request_all_open_trades(ib) -> tuple:
        """Fetch a fresh cross-client snapshot; failure must block a creation SELL."""
        previous_timeout = ib.RequestTimeout
        try:
            ib.RequestTimeout = 5
            return tuple(ib.reqAllOpenOrders())
        finally:
            ib.RequestTimeout = previous_timeout

    def _refresh_account_state(self, ib, contract, *, open_trades=None) -> None:
        accounts = tuple(str(item) for item in ib.managedAccounts())
        positions = {account: 0 for account in accounts}
        contract_id = int(getattr(contract, "conId", 0) or 0)
        for position in ib.positions():
            position_contract = getattr(position, "contract", None)
            same_contract = int(getattr(position_contract, "conId", 0) or 0) == contract_id
            same_symbol = (
                str(getattr(position_contract, "symbol", "") or "").upper() == "XOP"
                and str(getattr(position_contract, "secType", "") or "").upper() == "STK"
            )
            if same_contract or same_symbol:
                account = str(getattr(position, "account", "") or "")
                positions[account] = positions.get(account, 0) + int(float(position.position))

        # Account-update portfolio data is a useful fallback if the global
        # positions cache is briefly stale; prefer it for any matching account.
        portfolio = getattr(ib, "portfolio", None)
        for item in portfolio() if callable(portfolio) else ():
            item_contract = getattr(item, "contract", None)
            same_contract = int(getattr(item_contract, "conId", 0) or 0) == contract_id
            same_symbol = (
                str(getattr(item_contract, "symbol", "") or "").upper() == "XOP"
                and str(getattr(item_contract, "secType", "") or "").upper() == "STK"
            )
            if same_contract or same_symbol:
                account = str(getattr(item, "account", "") or "")
                positions[account] = int(float(item.position))

        with self._lock:
            session_refs = set(self._session_sell_commitments)
        external_pending = {account: 0 for account in accounts}
        unknown_pending = 0
        trades = ib.openTrades() if open_trades is None else open_trades
        for trade in trades:
            trade_contract = getattr(trade, "contract", None)
            trade_order = getattr(trade, "order", None)
            if str(getattr(trade_contract, "symbol", "") or "").upper() != "XOP":
                continue
            if str(getattr(trade_order, "action", "") or "").upper() != "SELL":
                continue
            order_ref = str(getattr(trade_order, "orderRef", "") or "")
            if order_ref in session_refs:
                continue
            account = str(getattr(trade_order, "account", "") or "")
            if not account and len(accounts) == 1:
                account = accounts[0]
            remaining = int(float(getattr(trade.orderStatus, "remaining", 0) or 0))
            if account not in external_pending:
                unknown_pending += max(0, remaining)
            else:
                external_pending[account] += max(0, remaining)
        if unknown_pending:
            for account in accounts:
                external_pending[account] += unknown_pending

        with self._lock:
            changed = (
                accounts != self._managed_accounts
                or positions != self._xop_positions
                or external_pending != self._xop_external_pending_sells
            )
            self._managed_accounts = accounts
            self._xop_positions = positions
            self._xop_external_pending_sells = external_pending
        if changed:
            self._emit_account_state()
