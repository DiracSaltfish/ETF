...
DEFAULT_CONFIG = {
    ...
    "qmt3_path": "",
    "ib_path": "",
    "ib_data_source_mode": "flex_auto",
    "ib_flex_start_date": (ib_data_source.latest_available_date() - timedelta(days=30)).isoformat(),
    "ib_flex_query_id": os.environ.get("IBKR_FLEX_QUERY_ID", ""),
    "ib_flex_token": "",
    "ib_flex_cache_dir": str(ROOT / "ib_auto_data"),
    "ib_flex_token_env": "IBKR_FLEX_TOKEN",
    "ib_flex_keychain_service": "ETFRedemptionCalculator.IBKRFlex",
    "fx_rate": "6.79635",
    "auto_refresh": True,
    "market_holidays": [],
...
def refresh_ib_flex_from_config(
    config: dict[str, object],
    *,
    downloader: Callable[[str, str, date, date], bytes] = ib_flex.download_statement,
) -> ib_data_source.AutoRefreshResult:
    start_text = str(config.get("ib_flex_start_date") or "").strip()
    try:
        start_date = date.fromisoformat(start_text)
    except ValueError:
        raise ValueError(f"IB Flex 起始日期格式无效：{start_text or '未配置'}") from None
    cache_dir = str(config.get("ib_flex_cache_dir") or DEFAULT_CONFIG["ib_flex_cache_dir"])
    return ib_data_source.refresh_flex_csv(
        start_date,
        str(config.get("ib_flex_query_id") or ""),
        cache_dir,
        configured_token=str(config.get("ib_flex_token") or ""),
        token_env=str(config.get("ib_flex_token_env") or "IBKR_FLEX_TOKEN"),
        keychain_service=str(
            config.get("ib_flex_keychain_service") or "ETFRedemptionCalculator.IBKRFlex"
        ),
...
class IbFlexRefreshWorker(QObject):
    finished = pyqtSignal(object)
    ...
    def _interruptible_sleep(self, seconds: float) -> None:
        if self._cancel_event.wait(max(0.0, float(seconds))):
            raise ib_flex.FlexError("IB Flex 拉取已取消")

    def _download(self, token: str, query_id: str, start_date: date, end_date: date) -> bytes:
        if self._cancel_event.is_set():
            raise ib_flex.FlexError("IB Flex 拉取已取消")
        return ib_flex.download_statement(
            token,
            query_id,
            start_date,
            end_date,
    ...
    def run(self) -> None:
        payload: dict[str, object] = {"result": None, "error": "", "cancelled": False}
        try:
            if self._cancel_event.is_set():
                payload["cancelled"] = True
            else:
                payload["result"] = refresh_ib_flex_from_config(
                    self.config,
                    downloader=self._download,
                )
        except Exception as exc:
            payload["error"] = str(exc)
            payload["cancelled"] = self._cancel_event.is_set()
        self.finished.emit(payload)
        QThread.currentThread().quit()
...
class SettingsDialog(QDialog):
    def __init__(self, config: dict[str, object], parent=None) -> None:
        ...
        self.ib_source_mode = QComboBox()
        self.ib_source_mode.addItem("IB Flex 自动拉取并更新本地 CSV", "flex_auto")
        self.ib_source_mode.addItem("从本地选择 CSV 文件", "local_csv")
        configured_mode = str(config.get("ib_data_source_mode") or "flex_auto")
        mode_index = self.ib_source_mode.findData(configured_mode)
        self.ib_source_mode.setCurrentIndex(mode_index if mode_index >= 0 else 0)

        try:
            configured_start = date.fromisoformat(str(config.get("ib_flex_start_date") or ""))
        except ValueError:
            configured_start = date.fromisoformat(str(DEFAULT_CONFIG["ib_flex_start_date"]))
        latest = ib_data_source.latest_available_date()
        configured_start = min(configured_start, latest)
        self.ib_flex_start_date = QDateEdit()
        self.ib_flex_start_date.setCalendarPopup(True)
        self.ib_flex_start_date.setDisplayFormat("yyyy-MM-dd")
        self.ib_flex_start_date.setMinimumDate(QDate(2000, 1, 1))
        self.ib_flex_start_date.setMaximumDate(QDate(latest.year, latest.month, latest.day))
        self.ib_flex_start_date.setDate(
            QDate(configured_start.year, configured_start.month, configured_start.day)
        )
        self.ib_flex_query_id = QLineEdit(str(config.get("ib_flex_query_id") or ""))
        self.ib_flex_query_id.setPlaceholderText("Activity Flex Query ID")
        self.ib_flex_token = QLineEdit(str(config.get("ib_flex_token") or ""))
        self.ib_flex_token.setEchoMode(QLineEdit.Password)
        self.ib_flex_token.setPlaceholderText("Flex Web Service Token")
        self.ib_flex_show_token = QCheckBox("显示")
        self.ib_flex_show_token.toggled.connect(
            lambda checked: self.ib_flex_token.setEchoMode(
                QLineEdit.Normal if checked else QLineEdit.Password
            )
        )
        self.ib_flex_token_row = QWidget()
        token_layout = QHBoxLayout(self.ib_flex_token_row)
        token_layout.setContentsMargins(0, 0, 0, 0)
        token_layout.addWidget(self.ib_flex_token, 1)
        token_layout.addWidget(self.ib_flex_show_token)
        self.ib_flex_cache = DirectoryPicker(
            "选择 IB 自动数据本地目录",
            str(config.get("ib_flex_cache_dir") or DEFAULT_CONFIG["ib_flex_cache_dir"]),
        )
        self.ib = FilePicker(
            "选择 IB 完整成交汇总",
            str(config.get("ib_path") or ""),
            "CSV (*.csv)",
        )
        self.ib_flex_note = QLabel(
            "程序启动和重新计算只读取本地缓存；点击主界面的“拉取IB”才会联网更新。"
            "Token 会按你的要求以明文保存在 config.json，请妥善保护和传输该文件。"
        )
        self.ib_flex_note.setWordWrap(True)
        ...
        form.addRow("IB 数据输入方式", self.ib_source_mode)
        form.addRow("自动拉取起始日期", self.ib_flex_start_date)
        form.addRow("Flex Query ID", self.ib_flex_query_id)
        form.addRow("Flex Token", self.ib_flex_token_row)
        form.addRow("自动 CSV 本地目录", self.ib_flex_cache)
        form.addRow("自动模式说明", self.ib_flex_note)
        form.addRow("本地 IB CSV", self.ib)
        ...
        self.ib_source_mode.currentIndexChanged.connect(self._update_ib_source_controls)
        self._update_ib_source_controls()

    def _update_ib_source_controls(self) -> None:
        automatic = self.ib_source_mode.currentData() == "flex_auto"
        self.ib_flex_start_date.setEnabled(automatic)
        self.ib_flex_query_id.setEnabled(automatic)
        self.ib_flex_token_row.setEnabled(automatic)
        self.ib_flex_cache.setEnabled(automatic)
        self.ib_flex_note.setEnabled(automatic)
        self.ib.setEnabled(not automatic)
    ...
    def values(self) -> dict[str, object]:
        return {
            ...
            "ib_data_source_mode": str(self.ib_source_mode.currentData()),
            "ib_flex_start_date": self.ib_flex_start_date.date().toString("yyyy-MM-dd"),
            "ib_flex_query_id": self.ib_flex_query_id.text().strip(),
            "ib_flex_token": self.ib_flex_token.text().strip(),
            "ib_flex_cache_dir": self.ib_flex_cache.value(),
...
class BasketDetailDialog(QDialog):
    def __init__(
        ...
        summary_rows = [
            ...
            (
                "IB实际闭合",
                f"{basket.ib_matched_qty:,} 股"
                + (
                    "（尾差放行）"
                    if basket.ib_open_tail_accepted or basket.ib_close_tail_accepted
                    else ""
                ),
            ),
            (
...
class RealtimePremiumTab(QWidget):
    def __init__(self, config: dict[str, object], parent=None) -> None:
        ...
        self._pcf_cache_key: tuple[date, tuple[int, int, int, int] | None] | None = None
        self._latest_valuation: realtime_premium.PremiumValuation | None = None
        self._latest_missing: tuple[str, ...] = ()
        self._valuation_error = ""
        self._last_render_key: tuple[object, ...] | None = None
        self._last_published_key: tuple[object, ...] | None = None
        self.started = False
        ...
        self._render_timer.timeout.connect(self._flush_realtime_render)
        self._publish_timer = QTimer(self)
        self._publish_timer.setSingleShot(True)
        self._publish_timer.setInterval(500)
        self._publish_timer.timeout.connect(self._publish_latest_snapshot)
        self._heartbeat_timer = QTimer(self)
        self._heartbeat_timer.setInterval(15_000)
        self._heartbeat_timer.timeout.connect(self._heartbeat_refresh)
    ...
    def _set_connection_badge(self, label: QLabel, text: str, state: str) -> None:
        style = self._connection_badge_style(state)
        if label.text() == text and label.toolTip() == text and label.styleSheet() == style:
            return
        label.setText(text)
    ...
    def refresh_pcf_estimate_cash_component(self) -> None:
        ...
        if trading_day.weekday() >= 5:
            cache_key = (trading_day, None)
            if self._pcf_cache_key == cache_key:
                return
            self._pcf_cache_key = cache_key
        ...
        item = self.pcf_store.build_fund_item(trading_day, szse_pcf.TARGET_FUND_CODE)
        xml_path = self.pcf_store.day_dir(trading_day) / item.cache_xml_path
        try:
        ...
        cache_key = (trading_day, signature)
        if self._pcf_cache_key == cache_key:
            return
    ...
    @staticmethod
    def _xop_quote_key(quote: realtime_premium.XopQuote | None) -> tuple[object, ...] | None:
        if quote is None:
            return None
        return quote.bid, quote.ask, quote.last, quote.market_data_type

    @staticmethod
    def _domestic_quote_key(quote: realtime_premium.SinaQuote | None) -> tuple[object, ...] | None:
        if quote is None:
            return None
        return (
            quote.symbol,
            ...
            quote.last,
            quote.market_time,
            tuple((level.price, level.volume) for level in quote.bids),
            tuple((level.price, level.volume) for level in quote.asks),
        )

    @staticmethod
    def _cfets_quote_key(quote: realtime_premium.CfetsQuote | None) -> tuple[object, ...] | None:
        if quote is None:
            return None
        return quote.rate, quote.trading_day, quote.quote_time

    def _semantic_state_key(self) -> tuple[object, ...]:
        return (
            self._xop_quote_key(self.xop_quote),
            self._domestic_quote_key(self.domestic_quote),
            self._cfets_quote_key(self.cfets_quote),
            self.pcf_valuation_day,
            self.pcf_estimate_cash_component,
            self.pcf_detail_error,
            self._latest_missing,
            self._valuation_error,
        )

    def update_xop_quote(self, quote: realtime_premium.XopQuote) -> None:
        previous_key = self._xop_quote_key(self.xop_quote)
        self.xop_quote = quote
        if self._xop_quote_key(quote) != previous_key:
            self._queue_realtime_refresh()

    def update_domestic_quote(self, quote: realtime_premium.SinaQuote) -> None:
        previous_key = self._domestic_quote_key(self.domestic_quote)
        self.domestic_quote = quote
        if self._domestic_quote_key(quote) != previous_key:
            self._queue_realtime_refresh()

    def update_cfets_quote(self, quote: realtime_premium.CfetsQuote) -> None:
        previous_key = self._cfets_quote_key(self.cfets_quote)
        self.cfets_quote = quote
        if self._cfets_quote_key(quote) != previous_key:
            self._queue_realtime_refresh()

    def _queue_realtime_refresh(self) -> None:
        self._recalculate_latest_valuation()
    ...
    def _schedule_snapshot_publish(self) -> None:
        if (
            self.shared_folder_path is not None
            and self._latest_valuation is not None
            and not self._publish_timer.isActive()
        ):
            self._publish_timer.start()

    def _heartbeat_refresh(self) -> None:
        previous_key = self._semantic_state_key()
        self._recalculate_latest_valuation()
        if self._semantic_state_key() != previous_key:
            if not self._render_timer.isActive():
                self._render_timer.start()
            self._schedule_snapshot_publish()
        elif self._latest_valuation is not None:
            self._publish_latest_snapshot(force=True)
    ...
    def write_shared_snapshot(self, value: realtime_premium.PremiumValuation) -> bool:
    ...
    def _publish_latest_snapshot(self, force: bool = False) -> None:
        value = self._latest_valuation
        if value is None:
            return
        publish_key = self._semantic_state_key()
        if not force and publish_key == self._last_published_key:
            return
        if self.write_shared_snapshot(value):
            self._last_published_key = publish_key

    def _recalculate_latest_valuation(self) -> None:
        ...
        self._latest_missing = tuple(missing)
        self._valuation_error = ""
        ...
        try:
            self._latest_valuation = realtime_premium.calculate_premium_valuation(
            ...
            self._valuation_error = str(exc)

    def refresh_tables(self) -> None:
        """Synchronously refresh once for manual/status changes and initial display."""
        self._recalculate_latest_valuation()
        self._flush_realtime_render(force=True)
        self._schedule_snapshot_publish()

    def _flush_realtime_render(self, force: bool = False) -> None:
        render_key = self._semantic_state_key()
        ...
        if self._latest_missing:
            self.update_premium_summary(None)
            self.populate_order_book()
            fill_table(
                self.valuation_table,
                ["状态", "缺少的数据", "说明"],
                [["等待实时数据", "；".join(self._latest_missing), "三项数据齐全后自动计算"]],
            )
            return
        if self._valuation_error:
            self.update_premium_summary(None)
            self.populate_order_book()
            fill_table(self.valuation_table, ["状态", "错误"], [["无法计算", self._valuation_error]])
            return
        value = self._latest_valuation
        if value is None:
            return
...
class XopCloseOrdersTab(QWidget):
    ...
    def __init__(
        self,
        tws_client: realtime_premium.TwsXopMarketData,
        intent: xop_close_orders.XopCloseIntent = xop_close_orders.XopCloseIntent.REDEMPTION_BUY,
        parent=None,
    ) -> None:
        super().__init__(parent)
        self.tws_client = tws_client
        self.intent = xop_close_orders.XopCloseIntent(intent)
        ...
        is_creation_sell = self.intent is xop_close_orders.XopCloseIntent.CREATION_SELL
        action_text = "买入" if self.intent.action == "BUY" else "融券卖出"
        short_risk = (
            "本页SELL允许建立或增加XOP空头，需要IB账户具备美股卖空权限、"
            "足够保证金和可借券；IB/TWS将最终决定接收、持有等待或拒绝。"
            if is_creation_sell
            else ""
        )
        warning = QLabel(
            f"业务方向：{self.intent.business_label}。实盘风险提示：本页会发送 "
            f"{self.intent.action} MKT DAY 条件单。市价单没有价格保护。{short_risk}"
        ...
        self.trade_date_edit = QLineEdit(datetime.now(xop_close_orders.NEW_YORK).strftime("%Y%m%d"))
        ...
        self.basket_count_spin = QSpinBox()
        self.basket_count_spin.setRange(
            1,
            2 if self.intent is xop_close_orders.XopCloseIntent.REDEMPTION_BUY else 1000,
        )
        self.basket_count_spin.setValue(1)
        self.basket_count_spin.setSuffix(" 张")
        self.basket_count_spin.setFixedWidth(90)
        self.total_qty_spin = QSpinBox()
        self.total_qty_spin.setRange(1, 1_000_000)
        self.total_qty_spin.setValue(990)
        self.total_qty_spin.setSuffix(" 股")
        # 键盘输入完成后再提交数值，避免逐位编辑时动态最小值重写文本。
        self.total_qty_spin.setKeyboardTracking(False)
        self.total_qty_spin.lineEdit().setReadOnly(False)
        self.total_qty_spin.setToolTip("可直接全选后输入数量；按回车或移开焦点后自动更新预览")
        self.time_plan_combo = QComboBox()
        if self.intent is xop_close_orders.XopCloseIntent.REDEMPTION_BUY:
            self.time_plan_combo.addItem(
                "原规则：1篮5笔 / 2篮10笔",
                "legacy",
            )
        self.time_plan_combo.addItem(
            "收盘均分：15:59:00 / :15 / :30 / :45",
            "balanced",
        )
        self.time_plan_combo.addItem(
            "保守提前：15:58:30 / :45 / 15:59:00 / :15",
            "conservative",
        )
        self.time_plan_combo.setMinimumWidth(290)
        self.account_combo = QComboBox()
        self.account_combo.addItem("连接IB后选择", "")
        self.account_combo.setMinimumWidth(145)
        self.total_qty_label = QLabel()
        self.total_qty_label.setObjectName("sourceHint")
        self.position_label = QLabel("XOP持仓：连接IB后显示")
        self.position_label.setObjectName("sourceHint")
        ...
        controls.addWidget(QLabel("国内篮子数"))
        controls.addWidget(self.basket_count_spin)
        controls.addWidget(QLabel("单篮目标平仓数量"))
        controls.addWidget(self.total_qty_spin)
        controls.addWidget(self.total_qty_label)
        controls.addWidget(QLabel("时间方案"))
        controls.addWidget(self.time_plan_combo)
        controls.addWidget(self.generate_button)
        controls.addStretch(1)

        account_controls = QHBoxLayout()
        account_controls.addWidget(QLabel("IB账户"))
        account_controls.addWidget(self.account_combo)
        account_controls.addWidget(self.position_label, 1)
        account_controls.addWidget(self.connect_button)
        account_controls.addWidget(self.disconnect_button)
        account_controls.addWidget(self.connection_status)
        ...
        self.unlock_checkbox = QCheckBox(
            f"解锁实盘{action_text}XOP（勾选后仍需逐笔确认）"
        )
        ...
        send_controls.addWidget(
            QLabel(
                f"固定参数：{self.intent.business_label} / XOP / {self.intent.action} / "
                "MKT / DAY / SMART→ARCA / outsideRth=false"
            )
        )
        ...
        self.basket_count_spin.valueChanged.connect(self.handle_basket_count_changed)
        self.total_qty_spin.valueChanged.connect(self.handle_total_qty_changed)
        self.time_plan_combo.currentIndexChanged.connect(self.handle_plan_changed)
        self.account_combo.currentIndexChanged.connect(self.handle_account_changed)
        ...
        self.tws_client.accountStateChanged.connect(self.handle_account_state)
        self.handle_account_state(self.tws_client.account_state())
        self.generate_preview(show_errors=False)

    def selected_basket_count(self) -> int:
        return self.basket_count_spin.value()
    ...
    def sync_basket_count_controls(self) -> None:
        ...
        trigger_times = self.selected_trigger_times()
        if (
            self.intent is xop_close_orders.XopCloseIntent.REDEMPTION_BUY
            and trigger_times is None
        ):
            ...
            order_count = len(trigger_times or ())
            minimum_base_qty = max(1, (order_count + basket_count - 1) // basket_count)
        self.total_qty_spin.setMinimum(minimum_base_qty)
        self.generate_button.setText(f"生成{order_count}张订单预览（不发送）")
        self.total_qty_label.setText(f"总目标 {self.actual_total_qty():,} 股")

    def handle_basket_count_changed(self, _index: int) -> None:
        self.sync_basket_count_controls()
        self.generate_preview(show_errors=False)

    def handle_total_qty_changed(self, _value: int) -> None:
        self.total_qty_label.setText(f"总目标 {self.actual_total_qty():,} 股")
        self.generate_preview(show_errors=False)

    def handle_plan_changed(self, _index: int) -> None:
        self.sync_basket_count_controls()
        self.generate_preview(show_errors=False)

    def selected_trigger_times(self) -> tuple[clock_time, ...] | None:
        plan_key = str(self.time_plan_combo.currentData() or "balanced")
        if (
            self.intent is xop_close_orders.XopCloseIntent.REDEMPTION_BUY
            and plan_key == "legacy"
        ):
            return None
        return xop_close_orders.four_order_trigger_times(plan_key)

    def current_account(self) -> str:
        return str(self.account_combo.currentData() or "")

    def handle_account_changed(self, _index: int) -> None:
        self.update_position_label(self.tws_client.account_state())
        self.generate_preview(show_errors=False)

    def handle_account_state(self, payload: object) -> None:
        if not isinstance(payload, dict):
            return
        accounts = tuple(str(item) for item in payload.get("accounts", ()))
        current = self.current_account()
        listed = tuple(
            str(self.account_combo.itemData(index) or "")
            for index in range(self.account_combo.count())
            if self.account_combo.itemData(index)
        )
        if listed != accounts:
            ...
            if not accounts:
                ...
                selected = current if current in accounts else accounts[0]
                self.account_combo.setCurrentIndex(self.account_combo.findData(selected))
            self.account_combo.blockSignals(False)
            self.generate_preview(show_errors=False)
        self.update_position_label(payload)

    def update_position_label(self, payload: object) -> None:
        if not isinstance(payload, dict):
            self.position_label.setText("XOP持仓：尚未同步")
            return
        account = self.current_account()
        states = payload.get("states", {})
        state = states.get(account, {}) if isinstance(states, dict) else {}
        if not account or not isinstance(state, dict):
            self.position_label.setText("XOP持仓：连接IB后显示")
            return
        position = int(state.get("position", 0))
        pending = int(state.get("pending_sell", 0))
        committed = int(state.get("session_sell", 0))
        if self.intent is xop_close_orders.XopCloseIntent.CREATION_SELL:
            projected = int(state.get("projected_position", position - pending - committed))
            self.position_label.setText(
                f"XOP当前持仓 {position:,} | 其他待成交SELL {pending:,} | "
                f"本次已确认SELL {committed:,} | 全部成交后预计 {projected:,}"
            )
        else:
            self.position_label.setText(f"XOP当前持仓 {position:,}")
    ...
    def generate_preview(self, _checked: bool = False, *, show_errors: bool = True) -> None:
        ...
        if show_errors:
            self.event_log.appendPlainText(
                ...
                f"{self.intent.business_label}，国内篮子 {basket_count} 张，单篮目标 {base_qty:,} 股，"
    ...
    def send_selected_orders(self) -> None:
        ...
        if self.intent is xop_close_orders.XopCloseIntent.CREATION_SELL:
            if not self.current_account():
                QMessageBox.warning(self, "未选择IB账户", "请先选择本批条件单使用的IB账户。")
                return
            if any(spec.account != self.current_account() for spec in specs):
                QMessageBox.warning(self, "账户已变化", "IB账户发生变化，请重新生成订单预览。")
                return
        for index, spec in enumerate(specs, start=1):
            try:
                ...
                continue
            short_confirmation = (
                "重要：这笔SELL可能建立或增加XOP空头，需要融券、保证金和卖空权限。\n\n"
                if self.intent is xop_close_orders.XopCloseIntent.CREATION_SELL
                else ""
            )
            text = (
                f"业务方向：{self.intent.business_label}\n"
                f"这是实盘市价条件单（第 {index}/{len(specs)} 笔）\n\n"
                f"IB账户：{spec.account or 'TWS默认账户'}\n"
    ...
    def handle_order_event(self, payload: object) -> None:
        ...
        if order_ref and not order_ref.startswith(self.intent.order_ref_prefix):
            return
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("ETF 赎回收益计算器 V4")
        ...
        self.input_cache = engine.CalculationInputCache()
        ...
        self._ib_auto_status = ""
        self._ib_auto_warning = ""
        self._shutting_down = False
        self._ib_refresh_thread: QThread | None = None
        self._ib_refresh_worker: IbFlexRefreshWorker | None = None
        ...
        self.xop_close_orders_tab = XopCloseOrdersTab(
            self.realtime_premium_tab.tws_client,
            xop_close_orders.XopCloseIntent.REDEMPTION_BUY,
        )
        self.xop_creation_close_orders_tab = XopCloseOrdersTab(
            self.realtime_premium_tab.tws_client,
            xop_close_orders.XopCloseIntent.CREATION_SELL,
        )
        tabs = QTabWidget()
        ...
        tabs.addTab(self.xop_close_orders_tab, "赎回国内-买入XOP条件单")
        tabs.addTab(self.xop_creation_close_orders_tab, "申购国内-卖出XOP条件单")
        ...
        self.pull_ib_button.clicked.connect(self.pull_ib_data)
    ...
    def input_paths(self) -> tuple[dict[str, str | None], str]:
        ...
        if str(self.config.get("ib_data_source_mode") or "flex_auto") == "flex_auto":
            ib_path = str(
                ib_data_source.cached_csv_path(
                    str(self.config.get("ib_flex_cache_dir") or DEFAULT_CONFIG["ib_flex_cache_dir"])
                )
            )
        else:
            ib_path = str(self.config.get("ib_path") or "").strip()
        return (
            {
    ...
    def update_source_label(self) -> None:
        ...
        mode = str(self.config.get("ib_data_source_mode") or "flex_auto")
        if mode == "flex_auto":
            cache_dir = str(self.config.get("ib_flex_cache_dir") or DEFAULT_CONFIG["ib_flex_cache_dir"])
            metadata = ib_data_source.load_metadata(cache_dir)
            latest = str(metadata.get("actual_end") or "尚未更新")
            ib_text = (
                f"Flex自动 {Path(ib).name}（起始 {self.config.get('ib_flex_start_date') or '--'}；"
                f"最新 {latest}）"
            )
        else:
            ib_text = f"本地 {Path(ib).name if ib else '--'}"
        self.source_label.setText(
            f"QMT1 {Path(qmt['QMT1']).name if qmt['QMT1'] else '--'} | "
            f"QMT2 {qmt2} | QMT3 {qmt3} | IB {ib_text}"
        )

    def update_ib_pull_button(self) -> None:
        if not hasattr(self, "pull_ib_button"):
            return
        running = self._ib_refresh_thread is not None and self._ib_refresh_thread.isRunning()
        automatic = str(self.config.get("ib_data_source_mode") or "flex_auto") == "flex_auto"
        self.pull_ib_button.setText("拉取中..." if running else "拉取IB")
        self.pull_ib_button.setEnabled(automatic and not running and not self._shutting_down)

    def pull_ib_data(self) -> None:
        if str(self.config.get("ib_data_source_mode") or "flex_auto") != "flex_auto":
            self.status_label.setText("当前使用本地 IB CSV 模式，无需拉取 Flex 数据。")
            return
        if self._ib_refresh_thread is not None and self._ib_refresh_thread.isRunning():
            return
        latest = ib_data_source.latest_available_date()
        self._ib_auto_status = ""
        self._ib_auto_warning = ""
        self.status_label.setText(f"正在后台通过 IB Flex 拉取至 {latest.isoformat()}...")

        thread = QThread(self)
        worker = IbFlexRefreshWorker(self.config)
        worker.moveToThread(thread)
        thread.started.connect(worker.run)
        worker.finished.connect(self.handle_ib_refresh_finished)
        worker.finished.connect(thread.quit)
        worker.finished.connect(worker.deleteLater)
        thread.finished.connect(self.handle_ib_refresh_thread_finished)
        thread.finished.connect(thread.deleteLater)
        self._ib_refresh_thread = thread
        self._ib_refresh_worker = worker
        thread.start()
        self.update_ib_pull_button()

    def handle_ib_refresh_finished(self, payload: object) -> None:
        data = dict(payload) if isinstance(payload, dict) else {}
        if self._shutting_down or bool(data.get("cancelled")):
            return
        error = str(data.get("error") or "")
        if error:
            self.status_label.setText(f"IB Flex 拉取失败：{error}。当前缓存未被覆盖。")
            return
        refresh_result = data.get("result")
        if not isinstance(refresh_result, ib_data_source.AutoRefreshResult):
            self.status_label.setText("IB Flex 拉取失败：未返回有效结果。当前缓存未被覆盖。")
            return
        self._ib_auto_status = refresh_result.status_text()
        self._ib_auto_warning = refresh_result.warning
        self.update_source_label()
        self.restart_watcher()
        self.refresh_timer.stop()
        self.calculate()

    def handle_ib_refresh_thread_finished(self) -> None:
        self._ib_refresh_thread = None
        self._ib_refresh_worker = None
        self.update_ib_pull_button()
    ...
    def calculate(self) -> None:
        ...
        if configuration_errors:
            ...
            self.status_label.setText(
                ...
                + (
                    "。Flex 缓存不存在，请点击上方“拉取IB”；程序启动不会自动联网。"
                    if mode == "flex_auto" and not Path(ib_path).expanduser().is_file()
                    else "。请点击右上角“数据源”完成配置。"
                )
            )
            self.refreshing = False
            self.restart_watcher()
        ...
        self.status_label.setText(
            ...
            + (
                f" | {self._ib_auto_warning or self._ib_auto_status}"
                if mode == "flex_auto" and (self._ib_auto_warning or self._ib_auto_status)
                else ""
            )
        )
        self.refreshing = False
    ...
    def shutdown(self) -> None:
        ...
        self.update_ib_pull_button()
        ib_worker = self._ib_refresh_worker
        if ib_worker is not None:
            ib_worker.cancel()
        ib_thread = self._ib_refresh_thread
        if ib_thread is not None and ib_thread.isRunning():
            ib_thread.requestInterruption()
            ib_thread.quit()
            ib_thread.wait(32_000)
    ...
    def populate_baskets(self, selected_id: str | None) -> None:
        ...
        for basket in self.result.baskets:
            ...
            ib_values = [
                (
                    f"{basket.ib_matched_qty:,}/{basket.hedge_target:,}"
                    + (
                        " 尾差放行"
                        if basket.ib_open_tail_accepted or basket.ib_close_tail_accepted
                        else ""
