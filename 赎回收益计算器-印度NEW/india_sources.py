...
def load_redemption_statement(
    ...
    for row_number, row in frame.iterrows():
        ...
        if day is None or qty <= 0:
            ...
            continue
        account = str(_value(row, frame, "account") or "").strip().upper()
        if account not in {"QMT1", "QMT2", "QMT3"}:
            fallback = str(default_account or "").strip().upper()
            if fallback not in {"QMT1", "QMT2", "QMT3"}:
                issues.append(StatementImportIssue(int(row_number) + 2, "无法识别赎回所属QMT账户", raw))
                continue
            account = fallback
