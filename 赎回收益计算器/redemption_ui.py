...
class BasketMappingTab(QWidget):
    ...
    def _ib_rows(self, start: date, end: date) -> list[dict[str, object]]:
        ...
        for pair in self.result.ib_self_closes:
            ...
            label = "IB自卖自平" if pair.opening.side == "SELL" else "IB自买自平"
...
class IbSelfCloseTab(QWidget):
    """Auditable IB residual pairing and trade-P&L view."""

    SELF_HEADERS = [
        "序号", "类型", "开仓时间", "平仓时间", "持有时长", "数量",
        "开仓价", "平仓价", "有利价差", "佣金USD", "盈亏USD", "盈亏RMB",
    ]
    ...
    def __init__(self, parent=None) -> None:
        ...
        guide = QLabel(
            "口径：先扣除赎回篮子、国内场内对冲、QMT3 承接和国内滚动头寸，"
            "再对剩余 IB 反向成交按时间 FIFO 自平。盈亏已计入按数量分摊的佣金，暂不分摊借券费。"
        )
        ...
        for key, title, tone in (
            ("matched", "自平配对", "normal"),
            ("usd", "自平盈亏 USD", "positive"),
            ("cny", "自平盈亏 RMB", "positive"),
            ("unmatched", "未闭合 IB", "accent"),
        ):
            metrics.addWidget(self._metric_card(key, title, tone), 1)

        self.self_table = configured_table()
        self.self_table.setAlternatingRowColors(False)
        self.self_table.setSortingEnabled(False)
        self.unmatched_table = configured_table()
        self.unmatched_table.setAlternatingRowColors(False)
        self.unmatched_table.setSortingEnabled(False)

        self_box = QGroupBox("自平交易明细（时间 FIFO）")
        self_layout = QVBoxLayout(self_box)
        self_layout.setContentsMargins(8, 12, 8, 8)
        self_layout.addWidget(self.self_table)
        unmatched_box = QGroupBox("未闭合风险头寸")
        unmatched_layout = QVBoxLayout(unmatched_box)
        unmatched_layout.setContentsMargins(8, 12, 8, 8)
        unmatched_layout.addWidget(self.unmatched_table)
        splitter = QSplitter(Qt.Vertical)
        splitter.addWidget(self_box)
        splitter.addWidget(unmatched_box)
    ...
    def _set_pnl_metric(self, key: str, value: Decimal, suffix: str) -> None:
        positive = value >= 0
        ...
        label.setText(f"{fmt_money(value)} {suffix}")
    ...
    def update_data(self, result: engine.CalculationResult) -> None:
        self.result = result
        self.metric_values["matched"].setText(
            f"{len(result.ib_self_closes):,} 组  /  {result.ib_self_close_qty:,} 股"
        )
        self._set_pnl_metric("usd", result.ib_self_pnl_usd, "USD")
        self._set_pnl_metric("cny", result.ib_self_pnl_cny, "RMB")
        self.metric_values["unmatched"].setText(
            f"SELL {result.unallocated_ib_sell_qty:,}  /  BUY {result.unallocated_ib_buy_qty:,}"
        )

        self_rows: list[list[object]] = []
        for pair in result.ib_self_closes:
            spread = (
                pair.opening.price - pair.closing.price
                if pair.opening.side == "SELL"
                else pair.closing.price - pair.opening.price
            )
            self_rows.append([
                pair.sequence,
                pair.direction,
        ...
        for row, pair in enumerate(result.ib_self_closes):
            background = QColor("#ecfdf3" if pair.trade_pnl_usd >= 0 else "#fef2f2")
            foreground = QColor("#14532d" if pair.trade_pnl_usd >= 0 else "#991b1b")
            for column in range(self.self_table.columnCount()):
                item = self.self_table.item(row, column)
                if item is not None:
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        tabs.addTab(self.ib_self_close_tab, "IB自平收益")
    ...
    def basket_asset_prediction_error_text(self, basket: engine.BasketResult) -> str:
        ...
        has_actual_refund = basket.actual_refund_day is not None or basket.manual_refund_applied
        if prediction is None or not has_actual_cash_difference or not has_actual_refund:
            return "--"
    ...
    def populate_summary(self) -> None:
        ...
        values = [
            ...
            ("IB自平收益 RMB", fmt_money(self.result.ib_self_pnl_cny)),
        ...
        for column, (name, value) in enumerate(values):
            ...
            elif name == "IB自平收益 RMB":
                pnl_value = self.result.ib_self_pnl_cny
    ...
    def populate_baskets(self, selected_id: str | None) -> None:
        ...
        if detailed:
            headers = [
                ...
                "来源", "合同编号", "国内成本", "现金差额", "退款", "篮子资产", "预估篮子资产", "预估偏差", "国内盈亏",
                "IB股数", "IB净盈亏USD", "IB盈亏RMB", "合计RMB", "IB映射",
            ]
        else:
            headers = [
                "轮次", "状态", "赎回日", "T+3", "T+6到账", "国内成本",
                "现金差额", "退款", "篮子资产", "预估篮子资产", "预估偏差", "国内盈亏", "IB股数", "IB净盈亏USD", "IB盈亏RMB",
                "合计RMB", "IB映射",
            ]
        rows = []
        payloads = []
    ...
    def populate_venue_closes(self) -> None:
        assert self.result is not None
        headers = [
            "日期", "时间", "账户", "合同编号", "数量", "卖出净额", "FIFO成本", "场内收益",
            "IB股数", "IB盈亏USD", "IB盈亏RMB", "合计RMB", "库存/IB缺口",
        ]
        rows = [
            [
                item.trade_day.isoformat(),
                ...
                fmt_money(item.cost),
                fmt_money(item.pnl),
                f"{item.hedge_target - item.ib_open_shortfall:,}/{item.hedge_target:,}",
                fmt_decimal(item.ib_trade_pnl_usd),
                fmt_money(item.ib_pnl_cny),
                fmt_money(item.pnl + item.ib_pnl_cny),
