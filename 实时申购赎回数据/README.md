...
解压后可直接双击 `ETF监控主机.app`。若需要登录后自动运行并在交易日 `09:15` 自动开始订阅，双击 `安装MacHome图形版.command`。安装脚本会把程序放到 `~/Applications`，以隐藏窗口方式登录启动；点击应用图标会唤醒已经运行的主界面，不会重复启动服务器。关闭主窗口只会收起到菜单栏，菜单栏图标中可重新显示或彻底退出。
...
- “设置”菜单提供 5 种内置原创短提示音，可立即试听；也可选择本机 WAV、MP3、M4A、AAC、OGG 或 FLAC 文件。音量、弹窗自动关闭时间和连续提醒冷却时间均可调整。
...
- 每个工作日 `08:00` 后为观察列表逐个拉取深圳 PCF；成功后按日期落盘，当天再次启动或查看详情不会重复请求。失败标的每 15 分钟重试、单标的每天最多 8 轮自动尝试；只抓结构化 XML，不额外抓 TXT。底层请求最少间隔 8 秒，并继承 V3 拉取器的 403/429/503 冷却保护。
...
- 双击桌面客户端或网页中的标的，可查看 PCF 摘要和成分证券明细；也可手动点击“刷新 PCF”。
...
网页可拉取全量、维护观察列表、开始/停止监控，并通过 WebSocket 接收变化。浏览器首次需要点击“启用声音”；后台系统通知需要点击“允许系统通知”。
