...
- 申购篮子（实时申购份额 ÷ PCF 最小申购赎回单位）
- 申购份额
- 赎回篮子（实时赎回份额 ÷ PCF 最小申购赎回单位）
- 赎回份额

申购和赎回金额已从订阅 SQL 与主页中移除。Wind 返回的笔数不再上屏；主页使用 PCF 最小申购赎回单位把份额换算成篮子数，PCF 未就绪时显示“待确认”。份额按万位四位分组，例如 `300 0000 - 200 0000 = +100 0000`。轧差份额正数表示净申购，负数表示净赎回；它随份额变化自动刷新，但不会独立触发提醒。
